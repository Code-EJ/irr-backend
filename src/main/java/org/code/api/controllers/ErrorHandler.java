...
public class ErrorHandler {
    ...
    public ResponseEntity<?> handleCreatorUserInvalid(
        AuthError.CreatorUserInvalid exception
    ) {
        String errorCode = exception.isInvalidUUID()
            ? "invalid_creator_uuid"
    ...
    public ResponseEntity<?> handleEmailOccupied(
        AuthError.EmailOccupied exception
    ) {
        log.debug(
            "Attempt to register an email that is already occupied {}",
            exception.getEmail()
        );
    ...
    public ResponseEntity<?> handleInvalidToken(
        AuthError.InvalidToken exception
    ) {
    ...
    public ResponseEntity<?> handleExpiredToken(
        AuthError.ExpiredToken exception
    ) {
        log.debug(
    ...
    public ResponseEntity<?> handleWrongCredentials(
        AuthError.WrongCredentials exception
    ) {
        log.debug(
            exception.isUserValid()
    ...
    public ResponseEntity<?> handlePasswordTooLong(
        AuthError.PasswordTooLong exception
    ) {
        log.debug(
            "Attempt to register with a password exceeding 72 bytes: {} bytes",
            exception.getPasswordLength()
        );
        ...
        return ResponseEntity.badRequest().body(
            Map.of(
                ...
                "password_too_long",
                "message",
                "The provided password exceeds the maximum allowed length of 72 bytes.",
                "password_length",
                exception.getPasswordLength()
            )
        );
